...
TECNICO, LABOR, NODO, UBICACION = range(4)

def start(update:Update, context: CallbackContext):
    context.bot.send_message(chat_id=update.effective_chat.id, text="""Con el bot de Netuno puedes /reportar cualquier
    eventualidad c""")

def reporte(update:Update, context: CallbackContext):
	global nombres

	layout=[["R5","R6","R7"],["R8","R9","R11"],["R13","R16","R19"]]
...
def HFCorFTTH(update:Update, context: CallbackContext):
	
def paso1(update:Update, context: CallbackContext):
...
def paso4(update:Update, context: CallbackContext):
